...
function DlRows({ values, setValues, incidentDate }: { values: DlState; setValues: (next: DlState | ((prev: DlState) => DlState)) => void; incidentDate: string | null }) { const dateStatus = getStatus(values.validUpto, incidentDate); const rowTone = dateStatus === "Invalid" ? "text-red-600" : dateStatus === "Valid" ? "text-green-600" : "text-green-600"; return <div className="divide-y divide-[#E6EEF7] px-6 py-3"><div className="grid grid-cols-[36px_1fr_190px_28px] items-center gap-3 py-3"><p className="text-[12px] font-semibold text-[#071D49]">1.</p><p className="text-[13px] font-semibold text-[#071D49]">Licence Valid Upto</p><input name="licence_valid_upto" type="date" value={values.validUpto} onChange={(event) => setValues((prev) => ({ ...prev, validUpto: event.target.value }))} className={`h-9 w-full rounded-md border px-3 text-[12px] font-semibold text-[#071D49] ${dateStatus === "Invalid" ? "border-red-300 bg-red-50" : dateStatus === "Valid" ? "border-green-300 bg-white" : "border-[#C9D4E3] bg-white"}`} /><span className={`text-[18px] ${rowTone}`}>{dateStatus === "Invalid" ? "⚠" : "⚑"}</span><input type="hidden" name="dl_validity_status" value={dateStatus} /></div><RadioRow number={2} label="Inbound" name="dl_inbound" value={values.inbound} onChange={(value) => setValues((prev) => ({ ...prev, inbound: value }))} invalid={false} /><RadioRow number={3} label="Valid for Loss Vehicle" name="dl_valid_for_loss_vehicle" value={values.validForLossVehicle} onChange={(value) => setValues((prev) => ({ ...prev, validForLossVehicle: value }))} invalid={values.validForLossVehicle === "No"} /></div>; }
