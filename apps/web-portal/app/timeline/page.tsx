...
export default async function TimelinePage() {
  ...
  return <AppShell><PageHeader title="Claim status timeline" description="A chronological view of process milestones for auditability, insurer coordination, and customer communication." /><SearchFilterBar searchPlaceholder="Search timeline by claim no, status, actor, or notes" filterLabel="Timeline status" />{error ? <DataError message={error.message} /> : <DataTable rows={data ?? []} emptyTitle="No timeline events" emptyDescription="Status changes recorded in Supabase will appear here." columns={[{ header: "Claim", cell: (item) => item.claims ? <Link className="font-semibold text-navy-700" href={`/claims/${item.claims.id}`}>{item.claims.claim_no}</Link> : "—" }, { header: "From", cell: (item) => item.from_status ? <StatusBadge status={item.from_status} /> : "—" }, { header: "To", cell: (item) => <StatusBadge status={item.to_status} /> }, { header: "Actor", cell: (item) => item.actor?.full_name ?? "—" }, { header: "Recorded", cell: (item) => new Date(item.created_at).toLocaleString() }, { header: "Notes", cell: (item) => item.notes ?? "—" }]} />}</AppShell>;
}
