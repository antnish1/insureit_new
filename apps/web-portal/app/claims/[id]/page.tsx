...
export default async function ClaimDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <AppShell>
      <PageHeader title={`Claim ${claim.claim_no}`} description="Detailed live claim workspace for accident summary, documents, insurer coordination, surveyor activity, repair milestones, and settlement values." />
      <div className="grid gap-6 xl:grid-cols-3">
        ...
        <div className="space-y-6">
          ...
          <Card>
            ...
            <div className="mt-4 space-y-3">{(documents ?? []).length === 0 ? <EmptyState title="No documents uploaded" description="Uploaded claim documents from Supabase storage will appear here." /> : documents?.map((document) => <div className="rounded-xl bg-slate-50 px-3 py-2 text-sm" key={document.id}><div className="flex items-center justify-between gap-3"><span>{document.document_type}</span><StatusBadge status={document.verification_status} /></div><p className="mt-1 text-xs text-slate-500">{document.file_name}</p></div>)}</div>
          </Card>
        </div>
      </div>
    </AppShell>
